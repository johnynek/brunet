...
/*          else if (item.type == "udp-as")
	...
	DhtIP.ReleaseIP(dht, node.ipop_namespace, node.ip.ToString(), node.password);
      ...
      //      if(DhtIP.GetIP(dht, node.ipop_namespace, ip.ToString(), 6048000, brunet_id, node.password, out new_password)) {
      //want to try a null password, hoping that a new password is generated by callee
      if(DhtIP.GetIP(dht, node.ipop_namespace, ip.ToString(), 6048000, brunet_id, null, out new_password)) {
